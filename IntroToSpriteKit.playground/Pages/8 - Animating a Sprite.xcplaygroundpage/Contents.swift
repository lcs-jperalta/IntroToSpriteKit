...
rocket.position = CGPoint(x: 250, y: rocket.size.height / 2)
scene.addChild(rocket)

// Create an empty array of SKTexture objects
var liftOffTextures: [SKTexture] = []

// Now add the two images we need in the array
liftOffTextures.append(SKTexture(imageNamed: "rocket_0"))
liftOffTextures.append(SKTexture(imageNamed: "rocket_1"))
liftOffTextures.append(SKTexture(imageNamed: "rocket_2"))
liftOffTextures.append(SKTexture(imageNamed: "rocket_3"))
liftOffTextures.append(SKTexture(imageNamed: "rocket_4"))
liftOffTextures.append(SKTexture(imageNamed: "rocket_5"))


let actionLiftOffAnimation = SKAction.animate(with: liftOffTextures, timePerFrame: 0.1, resize: true, restore: false)
let actionMoveUpward = SKAction.moveBy(x: 0, y: 1000, duration: 25)
let liftOff = SKAction.group([actionLiftOffAnimation, actionMoveUpward])
rocket.run(liftOff)
